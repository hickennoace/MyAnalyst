...
- **2.4 Cohort & retention** ✅ DONE (2026-06-07) — `analyzeCohorts()` in `lib/cohort.ts`: when the data has a recurring entity id (customer/user/…) + a time column, it groups entities by their first-seen period and builds a cohort × offset retention grid (reusing the time-series cadence/period helpers). Only fires when entities actually recur (transactional one-row-per-id data returns undefined). Surfaced as a `CohortCard` retention heatmap. 2 tests.
