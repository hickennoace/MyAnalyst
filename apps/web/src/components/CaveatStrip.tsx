  ...
  return (
    <div className="card flex gap-3 border-amber-500/30 bg-amber-500/[0.06] p-4">
      ...
      <div className="min-w-0">
        <p className="text-xs font-semibold text-amber-200">Read these results with care</p>
        <ul className="mt-1 space-y-0.5 text-[11px] text-amber-200/80">
          ...
          {caveats?.map((c) => (
            <li key={c.column}>
              ...
              {c.severity === "bad" && <span className="ml-1 rounded bg-amber-500/20 px-1 text-[10px]">major</span>}
              {" "}— figures using it may be off.
            </li>
          ))}
        </ul>
        <p className="mt-1.5 text-[10px] text-amber-200/60">From the data-quality check below. Fix the source and re-run for firmer numbers.</p>
      </div>
    </div>
  );
}
