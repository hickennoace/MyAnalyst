...
export function HeroChart({ className = "" }: { className?: string }) {
  ...
  return (
    <svg viewBox="0 0 490 200" className={className} role="img" aria-label="Animated trend chart" fill="none">
      ...
      {/* soft gridlines */}
      {[40, 80, 120, 160].map((y) => (
        <line key={y} x1="20" y1={y} x2="470" y2={y} stroke="rgba(20,20,50,0.07)" strokeWidth="1" strokeDasharray="3 7" />
      ))}
      ...
      {PTS.filter((_, i) => i % 3 === 0 || i === PTS.length - 1).map((p, i) => (
        <circle key={i} cx={p[0]} cy={p[1]} r="4" fill="#ffffff" stroke="#8b5cf6" strokeWidth="2" className="hero-node" style={{ animationDelay: `${i * 0.4}s` }} />
      ))}

      {/* floating accent dots */}
      <circle cx="60" cy="40" r="2.5" fill="#ec4899" className="hero-dot" opacity="0.7" />
      <circle cx="430" cy="150" r="2" fill="#6366f1" className="hero-dot" style={{ animationDelay: "1.5s" }} opacity="0.7" />
      <circle cx="250" cy="30" r="2" fill="#06b6d4" className="hero-dot" style={{ animationDelay: "2.5s" }} opacity="0.6" />
    </svg>
  );
}
