...
└── lib/                  # the analysis engine (pure TypeScript)
    ...
    ├── inference.ts      # statsmodels-grade: t/F/χ² distributions, correlation sig (Pearson+Spearman),
    │                     #   OLS + multiple regression w/ CI+p, ANOVA, chi-square, BH-FDR, describe
    ├── chart-theme.ts    # shared ECharts theme (gradients, rounded bars, BI-tool look)
